...
namespace GunRev
{
    public class Cryptocurrency : PlayerItem
    {
        ...
        private void OnPreCollision(SpeculativeRigidbody myRigidbody, PixelCollider myCollider, SpeculativeRigidbody other, PixelCollider otherCollider)
        {
            ...
            if (hp <= 0)
            {
                Exploder.Explode(turretObject.GetComponent<tk2dSprite>().WorldCenter, GameManager.Instance.Dungeon.sharedSettingsPrefab.DefaultExplosionData, turretObject.GetComponent<tk2dSprite>().WorldCenter);
